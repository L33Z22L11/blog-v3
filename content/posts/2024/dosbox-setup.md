...
- 下载 DOSBox 并安装。
  ...
  - 如果你电脑具有较好的国际互联网接入环境，可以通过 WinGet 一键安装。
    :copy{prompt="PS>"}[winget install dosbox]
...
- :copy{prompt="CMD>"}[notepad %LocalAppData%\DOSBox\dosbox-0.74-3.conf]
- :copy{prompt="PS>"}[code $env:LOCALAPPDATA\DOSBox\dosbox-0.74-3.conf]
