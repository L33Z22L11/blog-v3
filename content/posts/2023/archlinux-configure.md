...
- 启动 SSH 服务
  :copy[sudo systemctl start sshd]
- 查看 IP 地址
  :copy[ip -br a]
- 在其他有浏览器的设备上通过 SSH 连接到这台设备（复制后按 Ctrl-Shift-V 粘贴内容）
  :copy{prompt="PS>"}[ssh 用户名@IP地址]
...
- 安装
  :copy[sudo pacman -S reflector]
- 使用 Reflector 更新源
  :copy[sudo reflector --verbose --country China --sort rate --save /etc/pacman.d/mirrorlist]

## 字体

- 中文字体
  :copy[sudo pacman -S ttf-sarasa-gothic]
- Nerd 字体 (zsh主题需要)
  :copy[sudo pacman -S ttf-nerd-fonts-symbols]
...
- 让 Linux 认为主板时间是当地时间，而不是 UTC
  :copy[sudo timedatectl set-local-rtc true]

> 如果不想这样设置，还可以在 Windows 中添加注册表项，让 Windows 认为主板时间是 UTC。但是不要在两个系统中同时设置，否则矫枉过正会导致切换系统时产生反向时间差。
>
> :copy{prompt="PS(管理员)>"}[reg add HKLM\SYSTEM\CurrentControlSet\Control\TimeZoneInformation /v RealTimeIsUniversal /t REG_DWORD /d 1]

## 显卡驱动

- 仅限主流 Nvidia 显卡
  :copy[sudo pacman -S nvidia]
...
- 如果显示 `couldn't communicate with the NVIDIA driver` 提示，可能是显卡驱动没有安装成功
  - 可以先卸载 Nvidia 相关软件包
  :copy[sudo pacman -Rs nvidia nvidia-utils]
  - 强制更新 Pacman 软件包列表
  :copy[sudo pacman -Syyuu]
  - 重新安装 Nvidia 驱动
  :copy[sudo pacman -S nvidia]
  - 重启
...
- 查看 CPU 线程数
  :copy[cat /proc/cpuinfo | grep processor | wc -l]
- 编辑 makepkg 配置文件
  :copy[sudo vim /etc/makepkg.conf]
...
:copy[sudo systemctl enable --now bluetooth]

## Yay

- 下载，需要先安装 Git。
  :copy[git clone http://aur.archlinux.org/yay.git && cd yay]
- 编译安装，不行就多试几次或者换个网络。
  :copy[GOPROXY=http://goproxy.cn makepkg -si]
- 安装后就能清除 Git 目录了。
  :copy[cd .. && rm -rf yay]
...
- Microsoft Edge
  :copy[yay -S microsoft-edge-dev-bin]
- Google Chrome
  :copy[yay -S google-chrome]

## VS Code

  :copy[yay -S visual-studio-code-bin]
...
- 安装 V2$(echo r)aya-bin
  :copy[yay -S v2$(echo r)aya-bin]
- 启动 V2$(echo r)aya 服务并设置开机启动
  :copy[sudo systemctl enable --now v2$(echo r)aya]

如果遇到问题，可以手动安装。

- 切换到 yay/v2$(echo r)aya 目录
  :copy[cd ~/.cache/yay/v2$(echo r)aya-bin]
- 编辑 PKGBUILD 文件
  :copy[vim PKGBUILD]
...
- 安装 Zsh 和常用插件
  :copy[sudo pacman -S zsh zsh-completions zsh-autosuggestions zsh-syntax-highlighting]
- 安装 Powerlevel10k 主题
  :copy[yay -S zsh-theme-powerlevel10k]
- 设置本用户的默认 Shell 为 Zsh
  :copy[chsh -s /usr/bin/zsh]
- 编辑 `~/.zshrc`
  :copy[vim ~/.zshrc]
- 向配置文件中添加以下内容
  ```sh [~/.zshrc]
...
- 安装 Fcitx5 和中文输入插件
  :copy[yay -S fcitx5-im fcitx5-chinese-addons]
- 配置环境变量
  :copy[sudo vim /etc/environment]
- 向环境变量添加以下内容
  ```ini [/etc/environment]
