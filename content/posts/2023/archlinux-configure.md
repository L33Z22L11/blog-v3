...
- 启动 SSH 服务
  :copy{prefix="$" code="sudo systemctl start sshd"}
- 查看 IP 地址
  :copy{prefix="$" code="ip -br a"}
- 在其他有浏览器的设备上通过 SSH 连接到这台设备（复制后按 Ctrl-Shift-V 粘贴内容）
  :copy{prefix="PS>" code="ssh 用户名@IP地址"}
...
- 安装
  :copy{prefix="$" code="sudo pacman -S reflector"}
- 使用 Reflector 更新源
  :copy{prefix="$" code="sudo reflector --verbose --country China --sort rate --save /etc/pacman.d/mirrorlist"}

## 字体

- 中文字体
  :copy{prefix="$" code="sudo pacman -S ttf-sarasa-gothic"}
- Nerd 字体 (zsh主题需要)
  :copy{prefix="$" code="sudo pacman -S ttf-nerd-fonts-symbols"}
...
- 让 Linux 认为主板时间是当地时间，而不是 UTC
  :copy{prefix="$" code="sudo timedatectl set-local-rtc true"}

> 如果不想这样设置，还可以在 Windows 中添加注册表项，让 Windows 认为主板时间是 UTC。但是不要在两个系统中同时设置，否则矫枉过正会导致切换系统时产生反向时间差。
>
> :copy{prefix="PS(管理员)>" code="reg add HKLM\SYSTEM\CurrentControlSet\Control\TimeZoneInformation /v RealTimeIsUniversal /t REG_DWORD /d 1"}

## 显卡驱动

- 仅限主流 Nvidia 显卡
  :copy{prefix="$" code="sudo pacman -S nvidia"}
...
- 安装完显卡驱动后重启，检测 Nvidia 显卡驱动是否正常工作
  :copy{prefix="$" code="nvidia-smi"}
- 如果显示 `couldn't communicate with the NVIDIA driver` 提示，可能是显卡驱动没有安装成功
  - 可以先卸载 Nvidia 相关软件包
  :copy{prefix="$" code="sudo pacman -Rs nvidia nvidia-utils"}
  - 强制更新 Pacman 软件包列表
  :copy{prefix="$" code="sudo pacman -Syyuu"}
  - 重新安装 Nvidia 驱动
  :copy{prefix="$" code="sudo pacman -S nvidia"}
  - 重启
...
:copy{prefix="$" code="sudo pacman -S git"}
...
- 编辑环境变量文件
  :copy{prefix="$" code="sudo vim /etc/environment"}
...
- 查看 CPU 线程数
  :copy{prefix="$" code="cat /proc/cpuinfo | grep processor | wc -l"}
- 编辑 makepkg 配置文件
  :copy{prefix="$" code="sudo vim /etc/makepkg.conf"}
...
## 启用蓝牙
  ...
  :copy{prefix="$" code="sudo systemctl enable --now bluetooth"}

## Yay

- 下载，需要先安装 Git。
  :copy{prefix="$" code="git clone http://aur.archlinux.org/yay.git && cd yay"}
- 编译安装，不行就多试几次或者换个网络。
  :copy{prefix="$" code="GOPROXY=http://goproxy.cn makepkg -si"}
- 安装后就能清除 Git 目录了。
  :copy{prefix="$" code="cd .. && rm -rf yay"}

## 浏览器

- Microsoft Edge
  :copy{prefix="$" code="yay -S microsoft-edge-dev-bin"}
- Google Chrome
  :copy{prefix="$" code="yay -S google-chrome"}

## VS Code

  :copy{prefix="$" code="yay -S visual-studio-code-bin"}
...
- 安装 V2$(echo r)aya-bin
  :copy{prefix="$" code="yay -S v2$(echo r)aya-bin"}
- 启动 V2$(echo r)aya 服务并设置开机启动
  :copy{prefix="$" code="sudo systemctl enable --now v2$(echo r)aya"}
...
- 安装 Zsh 和常用插件
  :copy{prefix="$" code="sudo pacman -S zsh zsh-completions zsh-autosuggestions zsh-syntax-highlighting"}
- 安装 Powerlevel10k 主题
  :copy{prefix="$" code="yay -S zsh-theme-powerlevel10k-bin-git"}
- 设置本用户的默认 Shell 为 Zsh
  :copy{prefix="$" code="chsh -s /usr/bin/zsh"}
- 编辑 `~/.zshrc`
...
- 安装 Fcitx5 和中文输入插件
  :copy{prefix="$" code="yay -S fcitx5-im fcitx5-chinese-addons"}
- 配置环境变量
  :copy{prefix="$" code="sudo vim /etc/environment"}
- 向环境变量添加以下内容
  ```ini [/etc/environment]
