...
除了最基础的命令行程序之外，还有一部分交互式命令行程序使用了 :tip[TUI]{tip="终端用户界面（Terminal User Interface），也称为基于文本的用户界面（Text-based User Interface）"}，它在基本的命令行中实现了图形化交互，一个众所周知的例子是经典的 :tip[Windows 的启动管理器]{tip="不太像？网站调为深色模式试试！"}。
